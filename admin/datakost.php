  ...
  <div style="margin-top: 1%;">
    <center>
      <table width="80%" bgcolor="white">
        <tr>
          <th>Kod Pemilik Kost</th>
          <th>Nama Kost</th>
          <th>Nama Pemilik Kost</th>
          <th></th>
        </tr>
        <?php
          $query = mysqli_query($Koneksi, "SELECT *FROM kost NATURAL JOIN pemilik_kost");
          while ($tampil = mysqli_fetch_array($query)) {
        ?>
        <tr style="text-align: center; border-bottom: 1px solid grey;">
          <td><?php echo $tampil['kode_kost'] ?></td>
          <td><?php echo $tampil['nama_kost'] ?></td>
          <td><?php echo $tampil['nm_pemilik_kost'] ?></td>
          <td><a href="hapuskost.php?get=<?php echo $tampil['kode_kost']?>" style="text-decoration: none; color: red;"> Hapus </a></td>
        </tr>
      <?php
      }
    ?>
